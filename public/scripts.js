...
td.forEach(ele => {
  ele.addEventListener('click', processModal);
})
...
function submitUpdates(name, evt) {
  const sendUpdate = document.querySelector('.sendButton');
  const valueInput = document.querySelector('.selectedValue');
  ...
  sendUpdate.addEventListener('click', (e) => {
    ...
    try {
      fetch(`http://localhost:8080/parts/${name}`, {
        method: 'PATCH',
